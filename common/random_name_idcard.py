...
def getdistrictcode():
    codelist = []
    with open(r'E:\Program Files (x86)\Jenkins\workspace\zj_webservice_test\common\districtcode.txt'
              ) as file:
        lines = file.readlines()  # 逐行读取
    for line in lines:
        if (line.lstrip().rstrip().strip() != ''
                and (line.lstrip().rstrip().strip())
                    [:6][-2:] != '00'):
...
def update():
    id_card = gennerator()
    name = true_name()
    ob.write('userinfo', 'id_card', id_card)
    ob.write('userinfo', 'name', name)
